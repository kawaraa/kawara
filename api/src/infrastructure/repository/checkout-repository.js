...
class CheckoutRepository {
  ...
  async checkItem(item, country) {
    ...
    const query = `SELECT t1.pictures, t2.price, t3.cost AS shippingCost, t3.country FROM store.product t1 JOIN store.type t2 ON t1.number = t2.productNumber JOIN store.shipping t3 ON t1.number = t3.productNumber WHERE t1.number = ? AND t1.name = ? AND t2.type = ? AND t2.size = ? AND t3.country = ? GROUP BY t3.country`;
